...
app.post('/api/auth/clerk-signup', express.json(), (req, res) => {
  ...
  // Generate customized refer_code (First/Last initials + 10OFF + 2-digit alphanumeric collision resolver)
  const sanitizedName = name.trim().replace(/[^a-zA-Z]/g, '').toUpperCase() || 'USER';
  const firstTwo = sanitizedName.substring(0, 2);
  const lastTwo = sanitizedName.length > 2 ? sanitizedName.substring(sanitizedName.length - 2) : sanitizedName.substring(0);
  const basePrefix = `${firstTwo}${lastTwo}`;
  
  const resolveCollision = (base: string, suffix: string = ''): string => {
    const attempt = `${base}10OFF${suffix}`;
    const taken = Array.from(usersDb.values()).some(u => u.custom_referral_code === attempt);
    if (taken) {
      const randomTwo = Math.random().toString(36).substring(2, 4).toUpperCase();
...
app.post('/api/upload', express.json({ limit: '50mb' }), (req, res) => {
  ...
  // Simulate uploading image to S3 bucket / CDN.
  // In reality, we could upload the data to S3 and return a URL.
  // We'll generate a fake CDN URL or return a placeholder avatar URL for simulation,
  // or just return the base64 string directly but pretend it's a URL.
  // To simulate CDN return, let's just pretend the base64 string is the data URI from the CDN for the frontend to render.
  setTimeout(() => {
    res.json({ cdnUrl: image });
  }, 1200);
});
...
app.patch('/api/users/preferences', express.json({ limit: '50mb' }), (req, res) => {
  ...
  const { selected_font_scale, compact_view_enabled, selected_theme, name, avatar } = req.body;
  const userEmail = session.email.toLowerCase().trim();
  let user = usersDb.get(userEmail);
